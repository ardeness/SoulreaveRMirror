...
async def consumer_handler(websocket, path, parent_conn):
    while True:
        message = await websocket.recv()
        parent_conn.send(message)

async def producer_handler(websocket, path, child_conn2):
    while True:
        if child_conn2.poll():
...
async def handler(websocket, path, parent_conn, child_conn2):
    ...
    producer_task = loop.create_task(producer_handler(websocket, path, child_conn2))

def assistant_process(parent_conn2, credentials, args):
    with Assistant(credentials, args.device_model_id) as assistant:
        ...
        for event in events:
            process_event(event, assistant.device_id, parent_conn2)
...
def process_event(event, device_id, parent_conn2):
    ...
    if event.type == EventType.ON_CONVERSATION_TURN_STARTED:
        print(subprocess.check_output(['/home/ardeness/bin/turnscreenon.sh']))
        msg = {'status':'listening'}
        parent_conn2.send(json.dumps(msg))
    ...
    if event.type == EventType.ON_RECOGNIZING_SPEECH_FINISHED:
        ...
        msg = {'status':'responding', 'request':event.args["text"]}
        parent_conn2.send(json.dumps(msg))
        ...
        for command, params in process_device_actions(event, device_id):
            ...
            if command == "action.devices.commands.OnOff":
                if params['on']:
                    ...
                    print(subprocess.check_output(['/home/ardeness/bin/turnscreenoff.sh']))

            if command == "action.acme.commands.musicplayer":
                print(event)
...
#    p2.start()
    start_server = websockets.serve(
            functools.partial(handler, parent_conn=parent_conn,child_conn2=child_conn2),
            '127.0.0.1',8765)

    t = threading.Thread(target=assistant_process, args=(parent_conn2, credentials, args,))
    t.start()
